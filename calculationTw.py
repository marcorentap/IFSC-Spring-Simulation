...
def getGlobalError(Array1, Array2):
    differences = [abs(Array1[i] - Array2[i]) for i in range(len(Array1))]
    globalError = max(differences)
    print(globalError)

A = GetApproximateValues(100, refs.ETSHM6_6_inf)
B = GetExactValues(100)
print("Approximate_Length: {}".format(len(A)))
print("GetExactValues_Length: {}".format(len(B)))
getGlobalError(A, B)
getGlobalError(B, A)
